...
public class RedisServiceImpl implements RedisService {
    ...
    public boolean execScriptTest() {
        ...
        Long result= redisTemplate.execute(script, Collections.singletonList("ttt"), new String[] {"2","2000"});
    ...
    @Override
    public String hmget(String key,String str) {
    	return (String)redisTemplate.opsForHash().get(key, str);
    }

    @Override
    public Map<String,String> hmgetall(String key){
    	Map<Object,Object> allmap= redisTemplate.opsForHash().entries(key);
    	Map<String,String> map=new HashMap<String,String>();
    	for(Object obj:allmap.keySet()){
    		map.put(obj.toString(), allmap.get(obj).toString());
    	}
    	return map;
    }
    ...
    public void hset(String key, String s, String v) {
        try{
            this.hset(key.getBytes(),s.getBytes(),v.getBytes(redisCode),0L);
        }catch (Exception e) {
            LOGGER.error("",e);
        }
    }
    ...
    public void hset(String key, String s, String v, long liveTime) {
        try{
            this.hset(key.getBytes(),s.getBytes(),v.getBytes(redisCode),liveTime);
        }catch (Exception e) {
            LOGGER.error("",e);
        }
    }
